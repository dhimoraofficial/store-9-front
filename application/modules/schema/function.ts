...
//  */
// export async function getEntityData(entity_name: string, entity_itemID: string, fields: string[], tenant: string, store: string) {
//     const url = `/v1/${tenant}/${store}/schema/entity/${entity_name}/${entity_itemID}?fields=${encodeURIComponent(fields?.join("|"))}`
//     const response = await APP_API.GET(url)
//     return response
// }
...
//  */
// export async function getMultipleEntityData(
//     entities: {
//         name: ENTITY_NAME,
//         for: string,
//         field: string[]
...
//         return getEntityData(_ent?.name, _ent?.for, (fields || _ent?.field), tenant!, store!)
//     });
